...
# Generate mathops.go, the lookup tables giving information about dalvik math operations by opcode
...
''')
    ...
    print('var UNARY = map[uint8]unaryOpInfo{')
    for i, code in enumerate(unary.split()):
        ...
        print('    0x{:02X}: unaryOpInfo{{{}, scalars.{}, scalars.{}}},'.format(i + 0x7b, code.upper(), srct, destt))
    print('}')

    print('var BINARY = map[uint8]binaryOpInfo{')
    for i, code in enumerate(binary.split()):
        ...
        print('    0x{:02X}: binaryOpInfo{{{}, scalars.{}, scalars.{}}},'.format(i + 0x90, code.upper(), st, st2))
    print('}')

    print('var BINARY_LIT = map[uint8]binary2OpInfo{')
    for i, code in enumerate(binlit.split()):
        print('    0x{:02X}: binary2OpInfo{{{}}},'.format(i + 0xd0, code.upper()))
    print('}')
