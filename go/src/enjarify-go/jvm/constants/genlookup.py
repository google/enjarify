...
''')
    ...
    for t in ('FLOAT', 'DOUBLE'):
        for suf in 'SIGN NAN INF NINF'.split():
            s = '{}_{}'.format(t, suf)
            print('const {} = {}'.format(s, hex(eval(s))))

    for name, et, d in zip('INTS LONGS FLOATS DOUBLES'.split(), 'int32 int64 uint64 uint64'.split(), [all_ints, all_longs, all_floats, all_doubles]):
        print('var {} = map[{}]string{{'.format(name, et))
        for k, v in sorted(d.items()):
            sv = ''.join(map('\\x{:02x}'.format, v))
            print('    {}: "{}",'.format(hex(k), sv))
        print('}')
